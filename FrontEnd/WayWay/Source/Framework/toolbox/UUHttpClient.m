...
+ (NSMutableURLRequest*) buildRequest:(UUHttpClientRequest*)request
{
    ...
    {
        NSURL* tmp = [NSURL URLWithString:request.url];
        fullUrl = [request.url stringByAppendingString:[self buildQueryString:request.queryArguments existing:tmp.query]];
    }
...
+ (NSString*) buildQueryString:(NSDictionary*)dictionary existing:(NSString*)existing
{
