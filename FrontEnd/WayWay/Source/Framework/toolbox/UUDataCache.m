        ...
        for (NSString* item in contents)
        {
            ...
            if (!err && attrs)
            {
                ...
                if (fileSize)
                {
                    ...
                    {
                        UUDebugLog(@"Clearing %@ from UUDataCache", item);
...
+ (void) uuPurgeExpiredContent
{
    ...
    if (!err && contents)
    {
        for (NSString* item in contents)
        {
            ...
            {
                UUDebugLog(@"Clearing %@ from UUDataCache", url);
...
+ (BOOL) uuDoesCachedDataExistForURL:(NSURL*)url
{
    NSString* path = [self uuCachePathForURL:url];
    if (path)
    {
